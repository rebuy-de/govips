...
## Project Status & Future of `govips`

`govips` was created to provide a fast image processing library for Go by wrapping `libvips`. However, it relies on manually maintained Go APIs that call high-level C functions not originally intended for direct binding. This has led to significant challenges:

* **Maintenance Burden:** Manually keeping up with the frequent and excellent innovations in `libvips` is unsustainable for our small team.
* **Stability Issues:** The manual binding approach has resulted in difficult-to-debug issues, including segmentation violations and memory leaks.
* **Incomplete API Coverage:** We cannot offer the full, rich feature set that `libvips` provides.
...
Using bindings generated by `vipsgen` solves all the problems above and is the right path forward for the Go community. We recommend it for all new and existing projects that need the full power and stability of `libvips`.
